...
# ============================================================
   ...
   # 🟢 CAPEX calculation for transmission Eq.8, CAPEX for HVDC is considered for all type of transmission 1671 EURO/MW.km
    def TIC_rule(model):
...
#   - Under the baseline (without Data cenntre), the model.TotalDem[c, t, k, h] should be replaced with model.TDem[c, t, k, h]
   ...
   #🟢 Maximum target for renewable, storae and nuclear in the line with TYNDP buildout policy Eq.28
    def Target_CapMax_rule(model,j,c,t):
        if t in [2,3,4,5,6] and j in ['Nuclear', 'WindOn', 'WindOff', 'Battery', 'Solar']:
            return model.CAP[j,c,t]<=1.05*model.TargetMax[j,c,t]
        return Constraint.Skip
    ...
    for c in model.c:
        for t in model.t:
            ...
            #model.CAPnew['Hydro', c, t].setub(0)
            #model.CAPnew['Gas', c, t].setub(0)
    ...
    def Build2_rule(model, j, c, t):
        if t in [2, 3, 4, 5, 6]:
            ...
            # Hydro-specific rule
            if j in ['Hydro']:
                return model.CAPnew[j, c, t] <= 1000 * model.BR[j, c, t]  # 1000 use to convert GW to MW
   ...
   #🟢 Conver power to energy for battery and pumphydro storage according to charging duration time  Eq. 43
    ...
    def PeakDemand_rule(model,t,k,h):
            return model.PeakDemand[t]>= sum(model.TDem[c, t, k, h] for c in model.c)
    ...
    def Em_target_rule(model,t,c):
      return 0.001 *model.em[t,c]   <= 0.001 *1000*(model.EmTarget[t,c]) 
